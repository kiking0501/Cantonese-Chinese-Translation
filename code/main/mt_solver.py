...
class Solver:
    ...
    def getModel(self, config, buckets, mode='train', reuse=False ):
        ...
        if mode == 'train':
            ...
            for bucket_num, bucket_dct in self.buckets.items():
                ...
                print ("-" * 5)
                encoder_outputs = self.model_obj.getEncoderModel(
                    config, mode='training', reuse=reuse, bucket_num=bucket_num)
                pred = self.model_obj.getDecoderModel(
                    config, encoder_outputs, is_training=True,
                    mode='training', reuse=reuse, bucket_num=bucket_num)
            ...
            self.encoder_outputs = encoder_outputs = self.model_obj.getEncoderModel(
                config, mode='inference', reuse=True)
            decoder_outputs_inference, encoder_outputs, self.alpha_inference = \
                self.model_obj.getDecoderModel(
                    config, encoder_outputs, is_training=False, mode='inference', reuse=True)
            self.decoder_outputs_inference = decoder_outputs_inference

            self.token_lookup_sequences_placeholder_list = \
                self.model_obj.token_lookup_sequences_placeholder_list
            self.token_lookup_sequences_decoder_placeholder_list = \
                self.model_obj.token_lookup_sequences_decoder_placeholder_list
            self.token_output_sequences_decoder_placeholder_list = \
                self.model_obj.token_output_sequences_decoder_placeholder_list
            self.mask_list = self.model_obj.masker_list
            if config['use_pointer']:
                self.token_output_sequences_decoder_inpmatch_placeholder_list = \
                    self.model_obj.token_output_sequences_decoder_inpmatch_placeholder_list
        else:
            ...
            self.decoder_outputs_inference, self.encoder_outputs, self.alpha_inference = \
                self.model_obj.getDecoderModel(
                    config, encoder_outputs, is_training=False, mode='inference', reuse=False)
    ...
    def trainModel(self, config, train_feed_dict, val_feed_dct, reverse_vocab, do_init=True):
        ...
        for bucket_num, bucket in enumerate(self.buckets):
            encoder_inputs, decoder_inputs, decoder_outputs, decoder_outputs_inpmatch = \
                train_feed_dict[bucket_num]
            #cost = self.model_obj.cost

            # if y is passed as (N, seq_length, 1): change it to (N,seq_length)
            if len(decoder_outputs.shape) == 3:
                decoder_outputs = np.reshape(
                    decoder_outputs, (decoder_outputs.shape[0], decoder_outputs.shape[1]))
            ...
            feed_dct = {token_lookup_sequences_placeholder: encoder_inputs,
                        token_output_sequences_decoder_placeholder: decoder_outputs,
                        token_lookup_sequences_decoder_placeholder: decoder_inputs}
            if config['use_pointer']:
            ...
            while step <= training_iters:
                num_of_batches = int((n+batch_size-1)/batch_size)
                ...
                if step % display_step == 0:
                    ...
                    loss = self.getLoss(
                        ...
                        token_lookup_sequences_placeholder, token_lookup_sequences_decoder_placeholder,
                        token_output_sequences_decoder_placeholder, masker,
                        token_output_sequences_decoder_inpmatch_placeholder,
    ...
    def runInference(self, config, encoder_inputs, decoder_ground_truth_outputs,
        ...
        if typ == "greedy":
            decoder_outputs_inference, encoder_outputs, alpha_inference = \
                np.array(
                    sess.run([self.decoder_outputs_inference, self.encoder_outputs, self.alpha_inference],
                             feed_dict=feed_dct))  # timesteps, N
    ...
    def solveAll(self, config, encoder_inputs, decoder_ground_truth_outputs, reverse_vocab,
        ...
        if inference_type == "greedy":
            ...
            batch_size = 8
        num_batches = int((len(encoder_inputs) + batch_size - 1) / batch_size)
    ...
    def getLoss(self, config,
                encoder_input_sequences, decoder_input_sequences, decoder_output_sequences,
                enc_inp_placeholder, dec_in_placeholder, dec_out_placeholder,
                mask_placeholder,
                token_output_sequences_decoder_inpmatch_placeholder, decoder_outputs_matching_inputs,
                loss_variable, sentinel_loss_variable, sess):  # Probabilities
        print (" getLoss ...... ", "=" * 5)
        batch_size = config['batch_size']
        num_batches = int((len(encoder_input_sequences) + batch_size - 1) / batch_size)
        loss = []
        sentinel_loss = []
        all_vals = []
        for i in range(num_batches):
            ...
            if len(cur_input_sequences) < batch_size:
                ...
                for j in range(gap):
                    cur_decoder_output_sequences = np.vstack(
                        (cur_decoder_output_sequences, cur_decoder_output_sequences[0]))
                    cur_decoder_input_sequences = np.vstack(
                        (cur_decoder_input_sequences, cur_decoder_input_sequences[0]))
                    cur_input_sequences = np.vstack(
                        (cur_input_sequences, cur_input_sequences[0]))
                    cur_decoder_outputs_matching_inputs = np.concatenate(
                        (cur_decoder_outputs_matching_inputs, np.array([cur_decoder_outputs_matching_inputs[0]])))
            feed_dct = {enc_inp_placeholder: cur_input_sequences,
                        dec_out_placeholder: cur_decoder_output_sequences,
                        dec_in_placeholder: cur_decoder_input_sequences}
            if config['use_pointer']:
