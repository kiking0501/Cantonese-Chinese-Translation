...
class RNNModel:
    ...
    def __init__(self, buckets_dict, mode='training',params={}):
        ...
        if mode == 'training':
            ...
            for bucket_num, bucket in buckets_dict.items():
                ...
                self.token_lookup_sequences_placeholder_list.append(
                    tf.placeholder("int32", [None, max_sentence_length],
                                   name="token_lookup_sequences"+str(bucket_num))
                )  # token_lookup_sequences
                max_sentence_length = bucket['max_output_seq_length']
                self.masker_list.append(
                    tf.placeholder("float32", [None, max_sentence_length],
                                   name="masker"+str(bucket_num))
                )
                self.token_output_sequences_decoder_placeholder_list.append(
                    tf.placeholder("int32", [None, max_sentence_length],
                                   name="token_output_sequences_decoder_placeholder"+str(bucket_num))
                )
                self.token_lookup_sequences_decoder_placeholder_list.append(
                    tf.placeholder("int32", [None, max_sentence_length],
                                   name="token_lookup_sequences_decoder_placeholder"+str(bucket_num))
                )  # token_lookup_sequences

                self.token_output_sequences_decoder_inpmatch_placeholder_list.append(
                    tf.placeholder(
                        "float32",
                        [None, bucket['max_output_seq_length'], bucket['max_input_seq_length']],
                        name="token_output_sequences_decoder_inpmatch_placeholder"+str(bucket_num))
                )
        #print "========== INIT OVER ============= "
    ...
    def getEncoderModel(self, config, bucket_num=-1, mode='training', reuse=False):
        ...
        #placeholders
        if mode == 'training':
            token_lookup_sequences_placeholder = token_lookup_sequences_placeholder = \
                self.token_lookup_sequences_placeholder_list[bucket_num]
        elif mode == 'inference':
            self.token_lookup_sequences_placeholder_inference = token_lookup_sequences_placeholder = \
                tf.placeholder("int32", [None, max_sentence_length], name="token_lookup_sequences")
            # token_lookup_sequences
        ...
        if reuse:
            ...
            self.encoder_token_emb_mat = token_emb_mat = self.initEmbeddings(
                'emb_encoder', token_vocab_size, embeddings_dim, reuse=reuse,
        ...
        # run lstm
        outputs_tensor = self.encoderRNN(
            inp, lstm_cell_size, config['batch_size'], max_sentence_length, reuse, mode=mode)
    ...
    def runDecoderStep(self, lstm_cell, cur_inputs, prev_cell_output, state, encoder_outputs, sentinel, reuse=False):
        alpha, sentinel_weight, context = self.attentionLayer(
            encoder_outputs, prev_cell_output, sentinel, reuse)  # alpha is (None, encoder_sequence_length)
        #inputs =  cur_inputs
    ...
    def initDecoderOutputVariables(self, lstm_cell_size, token_vocab_size):
        with tf.variable_scope('decoder_output', reuse=None) as scope:
            w_out = tf.get_variable(
                'w_out', shape=[lstm_cell_size, token_vocab_size],
            ...
            w_context_out = tf.get_variable(
                'w_context_out', shape=[lstm_cell_size, token_vocab_size],
                initializer=tf.random_normal_initializer(-1.0,1.0))
    ...
    def getDecoderOutput(self,
                         output, lstm_cell_size, token_vocab_size, out_weights, alpha_sentinel,
                         encoder_input_sequence, batch_size, vocab_size, context,
    ...
    def initEmbeddings(self, emb_scope, token_vocab_size, embeddings_dim,
                       pretrained_embeddings_are_trainable=True, reuse=False, pretrained_embeddings=None):
        ...
        with tf.variable_scope(emb_scope, reuse=reuse):
            if pretrained_embeddings.all() is not None:
                token_emb_mat = tf.get_variable(
                    "emb_mat", shape=[token_vocab_size, embeddings_dim], dtype='float',
                    initializer=tf.constant_initializer(np.array(pretrained_embeddings)),
                    trainable=pretrained_embeddings_are_trainable
                )
                token_emb_mat = tf.concat(
                    [tf.zeros([1, embeddings_dim]), tf.slice(token_emb_mat, [1, 0], [-1, -1])], axis=0)
            else:
                token_emb_mat = tf.get_variable(
                    "emb_mat", shape=[token_vocab_size, embeddings_dim], dtype='float')
                # 0-mask
    ...
    def greedyInferenceModel(self, params):
        ...
        for time_step in range(num_steps):
            ...
            inputs_current_time_step = tf.reshape(
                tf.nn.embedding_lookup(token_emb_mat, inp), [-1, embeddings_dim])
            if time_step > 0: tf.get_variable_scope().reuse_variables()

            (cell_output, state), alpha_cur, sentinel_weight, context = self.runDecoderStep(
                lstm_cell=lstm_cell, cur_inputs=inputs_current_time_step,
                encoder_outputs=encoder_outputs, prev_cell_output=cell_output,
                sentinel=sentinel, reuse=(time_step != 0), state=state)
            cur_outputs = self.getDecoderOutput(
                cell_output, lstm_cell_size, token_vocab_size, out_weights,
                (alpha_cur, sentinel_weight), encoder_input_sequence, batch_size,
                token_vocab_size, context)
        ...
        with tf.variable_scope("RNN"):
            if mode == 'training':
                ...
                for time_step in range(num_steps):
                    ...
                    (cell_output, state), alpha, sentinel_weight, context = self.runDecoderStep(
                        lstm_cell=lstm_cell, cur_inputs=inputs_current_time_step,
                        encoder_outputs=encoder_outputs, prev_cell_output=cell_output,
                        sentinel=sentinel, reuse=(time_step != 0), state=state)
                    cur_pred = self.getDecoderOutput(
                        cell_output, lstm_cell_size, token_vocab_size, out_weights,
                        (alpha, sentinel_weight), encoder_input_sequence, batch_size,
                        token_vocab_size, context)
                ...
                outputs, alpha = self.greedyInferenceModel(params)  # self.beamSearchInference(params)
                # self.greedyInferenceModel(params)
    ...
    def getDecoderModel(self, config, encoder_outputs, is_training=False, mode='training', reuse=False, bucket_num=0):
        ...
        #placeholders
        if mode == 'training':
            token_lookup_sequences_decoder_placeholder = \
                self.token_lookup_sequences_decoder_placeholder_list[bucket_num]
            masker = self.masker_list[bucket_num]
            token_output_sequences_placeholder = \
                self.token_output_sequences_decoder_placeholder_list[bucket_num]
            encoder_input_sequence = self.token_lookup_sequences_placeholder_list[bucket_num]  # encoder
            decoder_output_inpmatch_sequence = \
                self.token_output_sequences_decoder_inpmatch_placeholder_list[bucket_num]
        else:
        ...
        if reuse:
            ...
            if share_embeddings:
                ...
                self.decoder_token_emb_mat = token_emb_mat = self.initEmbeddings(
                    emb_scope, token_vocab_size, embeddings_dim,
                    reuse=reuse, pretrained_embeddings=pretrained_embeddings,
                    pretrained_embeddings_are_trainable=config['pretrained_embeddings_are_trainable'])
            ...
            if mode == 'inference':
                ...
                if is_training:
                    ...
                    cost = tf.nn.sparse_softmax_cross_entropy_with_logits(
                        logits=pred_masked, labels=token_output_sequences_placeholder)
                        # token_output_sequences_placeholder is N,timesteps. cost will be N, timesteps
